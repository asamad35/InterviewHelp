...
import fs from 'fs'
import { CONSTANTS, TVIEW } from '@/common/utils'
...
export class ProcessingManager {
  ...
  private initializeAiClient(): void {
    try {
      ...
      if (config.apiProvider === CONSTANTS.API_PROVIDERS.OPENAI) {
        if (config.apiKey) {
      ...
      } else if (config.apiProvider === CONSTANTS.API_PROVIDERS.GEMINI) {
        this.openaiClient = null
        if (config.apiKey) {
  ...
  public async processScreenshots(): Promise<void> {
    ...
    if (config.apiProvider === CONSTANTS.API_PROVIDERS.OPENAI && !this.openaiClient) {
      this.initializeAiClient()
    ...
    } else if (config.apiProvider === CONSTANTS.API_PROVIDERS.GEMINI && !this.geminiClient) {
      this.initializeAiClient()
